...
import { Table, Vr, MultiListPage, DetailsPage } from '../../../public/components/factory';
...
describe(SubscriptionTableRow.displayName, () => {
  ...
  it('renders column for subscription name', () => {
    expect(wrapper.find(Vr).childAt(0).shallow().find(ResourceLink).props().name).toEqual(subscription.metadata.name);
    expect(wrapper.find(Vr).childAt(0).shallow().find(ResourceLink).props().title).toEqual(subscription.metadata.name);
    expect(wrapper.find(Vr).childAt(0).shallow().find(ResourceLink).props().namespace).toEqual(subscription.metadata.namespace);
    expect(wrapper.find(Vr).childAt(0).shallow().find(ResourceLink).props().kind).toEqual(referenceForModel(SubscriptionModel));
  });

  it('renders actions kebab', () => {
    const menuArgs = [ClusterServiceVersionModel, subscription];
    expect(wrapper.find(Vr).find(ResourceKebab).props().kind).toEqual(referenceForModel(SubscriptionModel));
    expect(wrapper.find(Vr).find(ResourceKebab).props().resource).toEqual(subscription);
    expect(wrapper.find(Vr).find(ResourceKebab).props().actions[0]).toEqual(Kebab.factory.Edit);
    expect(wrapper.find(Vr).find(ResourceKebab).props().actions[1](...menuArgs).label).toEqual('Remove Subscription...');
    expect(wrapper.find(Vr).find(ResourceKebab).props().actions[1](...menuArgs).callback).toBeDefined();
    expect(wrapper.find(Vr).find(ResourceKebab).props().actions[2](...menuArgs).label).toEqual(`View ${ClusterServiceVersionModel.kind}...`);
    expect(wrapper.find(Vr).find(ResourceKebab).props().actions[2](...menuArgs).href).toEqual(`/k8s/ns/default/${ClusterServiceVersionModel.plural}/testapp.v1.0.0`);
  });

  it('renders column for namespace name', () => {
    expect(wrapper.find(Vr).childAt(1).shallow().find(ResourceLink).props().name).toEqual(subscription.metadata.namespace);
    expect(wrapper.find(Vr).childAt(1).shallow().find(ResourceLink).props().title).toEqual(subscription.metadata.namespace);
    expect(wrapper.find(Vr).childAt(1).shallow().find(ResourceLink).props().displayName).toEqual(subscription.metadata.namespace);
    expect(wrapper.find(Vr).childAt(1).shallow().find(ResourceLink).props().kind).toEqual('Namespace');
  });
  ...
  it('renders column for subscription state when update available', () => {
    ...
    expect(wrapper.find(Vr).childAt(2).shallow().text()).toContain('Upgrade available');
  });

  it('renders column for subscription state when unknown state', () => {
    expect(wrapper.find(Vr).childAt(2).shallow().text()).toEqual('Unknown');
  });
  ...
  it('renders column for subscription state when update in progress', () => {
    ...
    expect(wrapper.find(Vr).childAt(2).shallow().text()).toContain('Upgrading');
  });
  ...
  it('renders column for subscription state when no updates available', () => {
    ...
    expect(wrapper.find(Vr).childAt(2).shallow().text()).toContain('Up to date');
  });

  it('renders column for current subscription channel', () => {
    expect(wrapper.find(Vr).childAt(3).shallow().text()).toEqual(subscription.spec.channel);
  });

  it('renders column for approval strategy', () => {
    expect(wrapper.find(Vr).childAt(4).shallow().text()).toEqual('Automatic');
  });
});
