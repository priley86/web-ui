...
describe(InstallPlanTableRow.displayName, () => {
  ...
  it('renders resource kebab for performing common actions', () => {
    expect(wrapper.find(Vr).find(ResourceKebab).props().actions).toEqual(Kebab.factory.common);
  });

  it('renders column for install plan name', () => {
    expect(wrapper.find(Vr).childAt(0).find(ResourceLink).props().kind).toEqual(k8s.referenceForModel(InstallPlanModel));
    expect(wrapper.find(Vr).childAt(0).find(ResourceLink).props().namespace).toEqual(testInstallPlan.metadata.namespace);
    expect(wrapper.find(Vr).childAt(0).find(ResourceLink).props().name).toEqual(testInstallPlan.metadata.name);
    expect(wrapper.find(Vr).childAt(0).find(ResourceLink).props().title).toEqual(testInstallPlan.metadata.uid);
  });

  it('renders column for install plan namespace', () => {
    expect(wrapper.find(Vr).childAt(1).find(ResourceLink).props().kind).toEqual('Namespace');
    expect(wrapper.find(Vr).childAt(1).find(ResourceLink).props().title).toEqual(testInstallPlan.metadata.namespace);
    expect(wrapper.find(Vr).childAt(1).find(ResourceLink).props().displayName).toEqual(testInstallPlan.metadata.namespace);
  });

  it('render column for install plan components list', () => {
    expect(wrapper.find(Vr).childAt(2).find(ResourceLink).props().kind).toEqual(k8s.referenceForModel(ClusterServiceVersionModel));
    expect(wrapper.find(Vr).childAt(2).find(ResourceLink).props().name).toEqual(testInstallPlan.spec.clusterServiceVersionNames.toString());
    expect(wrapper.find(Vr).childAt(2).find(ResourceLink).props().namespace).toEqual(testInstallPlan.metadata.namespace);
  });

  it('renders column for parent subscription(s) determined by `metadata.ownerReferences`', () => {
    expect(wrapper.find(Vr).childAt(3).find(ResourceLink).length).toEqual(1);
  });

  it('renders column for install plan status', () => {
    expect(wrapper.find(Vr).childAt(4).shallow().text()).toEqual(testInstallPlan.status.phase);
  });
  ...
  it('renders column with fallback status if `status.phase` is undefined', () => {
    ...
    expect(wrapper.find(Vr).childAt(4).shallow().text()).toEqual('Unknown');
    expect(wrapper.find(Vr).childAt(2).find(ResourceIcon).length).toEqual(1);
    expect(wrapper.find(Vr).childAt(2).find(ResourceIcon).at(0).props().kind).toEqual(k8s.referenceForModel(ClusterServiceVersionModel));
  });
});
